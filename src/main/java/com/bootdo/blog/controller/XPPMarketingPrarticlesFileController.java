...
*/
@Controller
@RequestMapping("/blog/xppContent_cp")
public class XPPMarketingPrarticlesFileController extends BaseController{
     ...
     */
    @GetMapping()
    @RequiresPermissions("blog:xppContent_cp:xppContent_cp")
    String xppContent() {
        return "blog/xppContent_cp/xppContent_cp";
    }
    ...
    String edit(@PathVariable("bvid") Long bvid, Model model) {
        ...
        return "blog/xppContent_cp/edit";
    }
    ...
    public R save(@RequestParam("avatar_file") MultipartFile file,XppBrandVisualDo xppContent,HttpServletRequest request) {
        ...
        if (StringUtils.isNoneEmpty(file.getOriginalFilename())) {
            ...
            try {
                FileUtil.uploadFile(file.getBytes(), bootdoConfig.getUploadPath()+"XPP/", fileName);
            } catch (Exception e) {
                return R.error();
            }
    ...
    public R download(@PathVariable("bvid")Long bvid ,HttpServletResponse response) {
        ...
        try {
            if (StringUtils.isNotEmpty(xppBrandVisualDo)) {
                ...
                //文件下载路径
                String path = bootdoConfig.getUploadPath()+"XPP/" + fileurl.replace("/files/", "");
                //下载文件
