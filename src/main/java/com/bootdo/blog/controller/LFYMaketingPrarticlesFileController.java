...
/**
* @Description:    LFY file controller
* @Author:         cxg
* @CreateDate:     10:08 2018/6/15
* @Version:        1.0
*/
@Controller
@RequestMapping("/blog/lfyContent_cp")
public class LFYMaketingPrarticlesFileController extends BaseController{
     ...
     */
    @GetMapping()
    @RequiresPermissions("blog:lfyContent_cp:lfyContent_cp")
    String xppContent() {
        return "blog/lfyContent_cp/lfyContent_cp";
    }
    ...
    String edit(@PathVariable("bvid") Long bvid, Model model) {
        ...
        return "blog/lfyContent_cp/edit";
    }
    ...
    public R save(@RequestParam("avatar_file") MultipartFile file,XppBrandVisualDo xppContent,HttpServletRequest request) {
        ...
        if (StringUtils.isNoneEmpty(file.getOriginalFilename())) {
            ...
            try {
                FileUtil.uploadFile(file.getBytes(), bootdoConfig.getUploadPath()+"LFY/", fileName);
            } catch (Exception e) {
                return R.error();
            }
    ...
    public R download(@PathVariable("bvid")Long bvid ,HttpServletResponse response) {
        ...
        try {
            if (StringUtils.isNotEmpty(xppBrandVisualDo)) {
                ...
                //文件下载路径
                String path = bootdoConfig.getUploadPath()+"LFY/" + fileurl.replace("/files/", "");
                //下载文件
